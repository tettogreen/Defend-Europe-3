...
namespace SmartSpawn {
	...
	public class SmartSpawnInfluenceMap : MonoBehaviour {
		...
		[Tooltip("The spawn object int the SmartSpawner will be override with the one we use to influence smartspawn scripts. " +
			"Can be used for correct influence spawn object reuse on wave reset.")]
		public bool overrideSpawnObjectInSpawner;
		...
		void HandleOnWaveReset (SmartSpawnScript spawnerWithWaveReset, int waveNumber)
		{
			...
			if (waveToPerformInfluenceOn == waveNumber || waveToPerformInfluenceOn == -1)
			{
				...
				if (currentlyInfluencedSpawnScripts.ContainsKey (spawnerWithWaveReset))
				{
					...
					if (overrideSpawnObjectInSpawner)
					{
						spawnerWithWaveReset.spawnObject = influenceSpawnObject;
					}
				}
			}
		}
