...
//			}
		...
		public void Spawn() {
			...
			{	
				if(useMaxPointSpawns == false || spawnCheckScript.maxSpawnsLeft > 0)
				{
					...
					spawnLocation = spawnCheckScript.spawnPivot ? spawnCheckScript.spawnPivot : spawnPoints[randomPick];
		...
		protected virtual void SpawnItem(Vector3 spawnLocation, int index) {
			...
			if(_gameObject != null){
				...
				GameObject newItem =  LeanPool.Spawn(_gameObject, spawnLocation, _gameObject.transform.rotation) as GameObject;
