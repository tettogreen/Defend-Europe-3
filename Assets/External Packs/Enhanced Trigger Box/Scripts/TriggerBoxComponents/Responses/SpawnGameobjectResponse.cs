        ...
        public override bool ExecuteAction()
        {
            if (prefabToSpawn)
            {
                ...
                if (!string.IsNullOrEmpty(newInstanceName))
                {
                    ...
                    {
                        GameObject newobj = LeanPool.Spawn(prefabToSpawn, customPositionRotation.position, customPositionRotation.rotation) as GameObject;
                        newobj.name = newInstanceName;
                    }
                    else
                    {
						GameObject newobj =  LeanPool.Spawn(prefabToSpawn) as GameObject;
                        newobj.name = newInstanceName;
                    }
                }
                else
                {
                    if (customPositionRotation)
                    {
						LeanPool.Spawn(prefabToSpawn, customPositionRotation.position, customPositionRotation.rotation);
                    }
                    else
                    {
						LeanPool.Spawn(prefabToSpawn);
                    }
                }
            }
